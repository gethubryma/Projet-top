...
#include <assert.h>
#include <stdlib.h>
//correction de quelques erreurs d'allocation mémoire 
mesh_t mesh_new(usz dim_x, usz dim_y, usz dim_z, mesh_kind_t kind) {
    usz const ghost_size = 2 * STENCIL_ORDER;

    cell_t*** cells = malloc((dim_x + ghost_size) * sizeof(cell_t**));//mod
    if (cells == NULL) {
    ...
    for (usz i = 0; i < dim_x + ghost_size; ++i) {
        ...
        for (usz j = 0; j < dim_y + ghost_size; ++j) {
            cells[i][j] = malloc((dim_z + ghost_size) * sizeof(cell_t)); /*malloc((dim_z + ghost_size) * sizeof(cell_t))*/;
            if (cells[i][j] == NULL) {
                error(
                    "failed to allocate dimension Z of mesh of size %zu bytes",  (dim_z + ghost_size) * sizeof(cell_t)
                );
            }
        }
    }
